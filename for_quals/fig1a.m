...
plot(params.sigma_h,Q,'k','LineWidth',1,'DisplayName','Coupled'); ylabel('channel discharge, $Q$','Interpreter','latex');title('Nondimensionalized steady state variables','Interpreter','latex');hold on;
set(gca,'Xticklabel',[]);
ax2 = subplot('position',[.1 .625 .8 .15]);
plot(params.sigma_h,N,'k','LineWidth',1,'DisplayName','Coupled');ylabel('effective pressure, $N$','Interpreter','latex');hold on;
set(gca,'Xticklabel',[]);
ax3 = subplot('Position',[.1 .45 .8 .15]);
plot(params.sigma_h,S,'k','LineWidth',1,'DisplayName','Coupled');ylabel('channel area, $S$','Interpreter','latex');hold on;
set(gca,'Xticklabel',[]);
ax4 = subplot('Position',[.1 .275 .8 .15]);    
plot(params.sigma_elem,h,'k','LineWidth',1,'DisplayName','Coupled');ylabel('ice thickness, $h$','Interpreter','latex');hold on;
set(gca,'Xticklabel',[]);
ax5 = subplot('Position',[.1 .1 .8 .15]);
plot(params.sigma,u,'k','LineWidth',1,'DisplayName','Coupled');hold on;ylabel('ice velocity, $u$','Interpreter','latex');xlabel('normalized distance','Interpreter','latex');ylim([0 45])
