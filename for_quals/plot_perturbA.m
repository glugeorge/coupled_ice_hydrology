...
ylim([210 250]);
xlim([0 5000]);
plot(ts,ones(size(ts)).*xg_f*params.x0/1e3, 'k--');
plot(time_to_ss*ones(1,2),[200 xg_f*params.x0/1e3], 'k--');
ax2 = subplot(3,4,5);
...
plot(ts,xgs.*params.x0./1e3,'linewidth',3);xlabel('time [yr]','Interpreter','latex');ylabel('Grounding line position [km]','Interpreter','latex');title('A = 2.9e-25; Constant-N Model','Interpreter','latex')
hold on;
ylim([210 250]);
xlim([0 5000]);
plot(ts,ones(size(ts)).*xg_f*params.x0/1e3, 'k--');
...
xlim([0 5000]);
plot(ts,ones(size(ts)).*xg_f*params.x0/1e3, 'k--');
plot(time_to_ss*ones(1,2),[195 xg_f*params.x0/1e3], 'k--');
ax8 = subplot(3,4,7);
contourf(ts,params.sigma_elem,hs'.*params.h0);colorbar;xlabel('time [yr]','Interpreter','latex');ylabel('$\sigma$','Interpreter','latex');set(gca,'Ydir','Reverse');
...
plot(ts,xgs.*params.x0./1e3,'linewidth',3);xlabel('time [yr]','Interpreter','latex');ylabel('Grounding line position [km]','Interpreter','latex');title('A = 4.9e-25; Constant-N Model','Interpreter','latex')
hold on;
ylim([195 210]);
xlim([0 5000]);
plot(ts,ones(size(ts)).*xg_f*params.x0/1e3, 'k--');
