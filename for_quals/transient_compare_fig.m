...
title('Comparing modeled transient evolution over 5 years','Interpreter','latex')

nexttile;
plot_profile(results_constcoulomb,'Static-N',results_constcoulomb.hs(:,end).*results_constcoulomb.params.h0,'h (m)','-',results_constcoulomb.params.sigma_elem);hold on;
...
plot_profile(results,'Coupled',results.hs(:,end).*results.params.h0,'ice thickness, h (m)','-',results.params.sigma_elem);
xlim([1300 1375]); set(gca,'Xticklabel',[]);

nexttile;
plot_profile(results_constcoulomb,'Static-N',results_constcoulomb.us(:,end).*results_constcoulomb.params.u0*60*60*24*365,'u (m y$^{-1}$)','-',results_constcoulomb.params.sigma);hold on;
...
plot_profile(results,'Coupled',results.us(:,end).*results.params.u0*60*60*24*365,'ice velocity, u (m y$^{-1}$)','-',results.params.sigma);
...
plot_profile(results,'Coupled',results.Ns(:,end).*results.params.N0,'effective pressure, N (Pa)','-',results.params.sigma_h);
