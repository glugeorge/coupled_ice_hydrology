...
function F = hydro_eqns(QNS,params)
    ...
    fs(1) = abs(Q(1)).^3./(S(1).^(8/3)) - ... 
                        S(1).*N(1).^3 + ...
                        (sigma(1)*(xg-xg_old)/dt - params.beta.*params.u(1)).*(S(2)-S(1))./(xg*params.dsigma_h(1)) - ...
                        (S(1)-params.S_old(1))./dt; % one sided differece
    fs(2:Nx-1)= abs(Q(2:Nx-1)).^3./(S(2:Nx-1).^(8/3)) - ... 
                        S(2:Nx-1).*N(2:Nx-1).^3 + ...
                        (sigma(2:Nx-1).*(xg-xg_old)./dt - params.beta.*params.u(2:Nx-1)).*(S(3:Nx)-S(1:Nx-2))./(2*xg*params.dsigma_h(2:Nx-1)) - ...
                        (S(2:Nx-1)-params.S_old(2:Nx-1))./dt; 
    fs(Nx)= abs(Q(Nx)).^3./(S(Nx).^(8/3)) - ... 
                        S(Nx).*N(Nx).^3 + ...
                        (sigma(Nx).*(xg-xg_old)./dt - params.beta.*params.u(Nx)).*(S(Nx)-S(Nx-1))./(xg*params.dsigma_h(Nx-1)) - ...
                        (S(Nx)-params.S_old(Nx))./dt; % one sided differece
