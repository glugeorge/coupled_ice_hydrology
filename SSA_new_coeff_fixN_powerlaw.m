...
filename = 'schoof_retreat_highres_tiny.mat';
load(filename);
...
%% Invert for lambda parameter
% Lambda will be whatever constant fits in front of u such that everything
% is nondimensional 
N_interp = interp1(params.fixed_N_grid,N,params.sigma*xg,"linear","extrap");
params.lambda = (params.gamma.*N_interp.*(u./(u+N_interp.^params.n)).^(1./params.n))./u;
params.fixed_u_grid = params.sigma*xg;
...
function F = flowline_eqns(huxg,params)
    ...
    lambda = interp1(params.fixed_u_grid,params.lambda,sigma*xg,"linear","extrap");
    % If using variable C
