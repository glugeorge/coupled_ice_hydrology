...
param_num = 100;
As_ar = linspace(1e-23,1e-19,param_num);
u0s = As_ar.*(params.C*params.N0).^params.n;
...
%% Plotting
figure();
subplot(3,1,1);plot(As_ar,xgs.*params.x0./1e3,'linewidth',3);xlabel('time (yr)');ylabel('x_g');
subplot(3,1,2);contourf(As_ar,params.sigma_elem,hs'.*params.h0);colorbar;xlabel('time (yr)');ylabel('sigma');title('thickness (m)');set(gca,'Ydir','Reverse');
subplot(3,1,3);contourf(As_ar,params.sigma,us'.*u0s.*params.year);colorbar;xlabel('time (yr)');ylabel('sigma');title('velocity (m/yr)');set(gca,'Ydir','Reverse');

figure();
Q_dim = Qs.*params.Q0;
N_dim = Ns.*params.N0;
S_dim = Ss.*params.S0;
subplot(3,1,1);surface(As_ar,params.sigma_h,Q_dim',EdgeColor='None');colorbar;xlabel('time (yr)');ylabel('distance');title('Flow (m^3/s)');set(gca,'Ydir','Reverse')
subplot(3,1,2);surface(As_ar,params.sigma_h,N_dim',EdgeColor='None');colorbar;xlabel('time (yr)');ylabel('distance');title('Effective Pressure (Pa)');set(gca,'Ydir','Reverse')
subplot(3,1,3);surface(As_ar,params.sigma_h,S_dim',EdgeColor='None');colorbar;xlabel('time (yr)');ylabel('distance');title('Surface Area (m^2)');set(gca,'Ydir','Reverse')
